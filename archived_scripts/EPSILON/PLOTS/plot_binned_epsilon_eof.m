function [F1,F2]=plot_binned_epsilon_eof(Epsilon_class,title_string,F1,F2,Meta_Data)
...
minP=min(Epsilon_class.mPshear1_eof(:));
...
for i=1:length(Epsilon_class.bin)
    ...
    ll(i)=loglog(ax1,Epsilon_class.k,Epsilon_class.mPshear1_eof(i,:),'Color',cmap(i,:),'linewidth',2);
...
nanind=find(nansum(Epsilon_class.mPshear1_eof,2)>0);
...
plot(ax2,log10(Epsilon_class.bin),Epsilon_class.nbin1_eof,'-+k')
set(ax2,'fontsize',20)
...
minP=min(Epsilon_class.mPshear2_eof(:));
...
for i=1:length(Epsilon_class.bin)
    ...
    ll(i)=loglog(ax3,Epsilon_class.k,Epsilon_class.mPshear2_eof(i,:),'Color',cmap(i,:),'linewidth',2);
end
...
nanind=find(nansum(Epsilon_class.mPshear2_eof,2)>0);
legend_string=arrayfun(@(x) sprintf('log_{10}(\\epsilon)~%2.1f',x),log10(Epsilon_class.bin),'un',0);
hl=gridLegend(ll(nanind),2,legend_string{nanind},'location','northwest');
set(hl,'fontsize',10)
hl.Position=[.01 .75 .25 .25];
...
plot(ax4,log10(Epsilon_class.bin),Epsilon_class.nbin2_eof,'-+k')
set(ax4,'fontsize',20)
