function [epsi,ctd,alt,act,vnav,gps] = mod_som_read_epsi_files_vGPS(filename,Meta_Data)
...
% Add the other versions and a switch to choose between them later. For
% now, use 'v3' only.
...
if isempty(ind_efe_start)
    ...
    %epsi.epsitime and epsi.epsidnum will be created from epsi_timestamp once
...
%     Bipolar = @(FR,data) (FR/efe.gain/2*(double(data)/2.^(efe.bit_counts-1)-1));
    ...
    if epsi_timestamp(1)>1e9
        % epsitime - seconds since 1970
        % epsidnum - matlab datenum
        [epsi.time_s,epsi.dnum] = convert_timestamp(epsi_timestamp);
    else
        % epsitime - seconds since power on
...
if isempty(ind_sbe_start)
    ...
    % If timestamp has values like 1.6e12, it is in milliseconds since Jan
    % 1, 1970. Otherwise it's in milliseconds since the start of the record
    if ctd_timestamp(1)>1e9
        % ctdtime - seconds since 1970
        % ctddnum - matlab datenum
        [ctd.time_s,ctd.dnum] = convert_timestamp(ctd_timestamp);
    else
        % ctdtime - seconds since power on
    ...
    switch sbe.data.format
        case 'eng'
            ...
            if ~isfield(Meta_Data.PROCESS,'latitude')
                error('Need latitude to get depth from pressure data')
            end
            ctd.z    = sw_dpth(ctd.P,Meta_Data.PROCESS.latitude);
            ctd.dzdt = [0; diff(ctd.z)./diff(ctd.ctdtime)];
    end
...
if isempty(ind_alt_start)
    ...
    %alt.alttime and alt.altdnum will be created from alt_timestamp once
    %all its records are filled
...
if isempty(ind_vnav_start)
    ...
    %vnav.vnavtime and vnav.vnavdnum will be created from vnav_timestamp once
    %all its records are filled
