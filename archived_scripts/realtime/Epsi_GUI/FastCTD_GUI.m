...
FastCTD_GUI_data.saveDir = '/Volumes/FCTD Softwares used in BLT 2022/EPSILOMETER_FCTD/realtime/realtime_epsi'; %Save anything made here in this directory. Otherwise you may get errors later when using GUI for FCTD instead of Epsi
FastCTD_GUI_data.epsi_profile_dir = '/Volumes/EPSI_PROCESSING/0802_d1_ef1_pc2/profiles';
...
FastCTD_GUI_data.matDir = '/Volumes/FCTD_EPSI/FCTD_MAT/mat/';
FastCTD_GUI_data.saveDir = '/Volumes/FCTD Softwares used in BLT 2022/EPSILOMETER_FCTD/realtime/realtime_epsi'; %Save anything made here in this directory. Otherwise you may get errors later when using GUI for FCTD instead of Epsi
FastCTD_GUI_data.epsi_profile_dir = '/Volumes/EPSI_PROCESSING/0802_d2_ef1_pc2/profiles';
...
FastCTD_GUI_data.settings = struct(...
    ...
    FastCTD_GUI_data.availableData(25) = struct('type', 'compass',...
                                          ...
                                          'dispMin', '20',...
                                          'dispMax', '28');
    FastCTD_GUI_data.availableData(26) = struct('type', 'epsilon1',...
                                          ...
                                          'dispMin', '20',...
                                          'dispMax', '28');
    FastCTD_GUI_data.availableData(27) = struct('type', 'epsilon2',...
                                          ...
                                          'dispMin', '20',...
                                          'dispMax', '28');
    FastCTD_GUI_data.availableData(28) = struct('type', 'chi1',...
                                          'description','chi1',...
                                          'dispText','log10(chi)',...
                                          'dispUnit','',...
                                          'dataMin', -10,...
                                          ...
                                          'dispMin', '20',...
                                          'dispMax', '28');
    FastCTD_GUI_data.availableData(29) = struct('type', 'chi2',...
                                          'description','chi2',...
                                          'dispText','log10-chi2',...
                                          'dispUnit','',...
                                          'dataMin', -10,...
...
if isempty(Vars2PlotInFrames)
    Vars2PlotInFrames = {'density','temperature','epsilon','chi2'};
end

persistent VarsBoxNames;
if isempty(VarsBoxNames)
    VarsBoxNames = {'Dens','Temp','Eps1','Chi2'};
end
...
% ~~~~~~~~~~~
% Add microstructure data from Epsi griddedProfiles to myFCTD_GridData
% ~~~~~~~~~~~
grid_file = dir(fullfile(FastCTD_GUI_data.epsi_profile_dir,'griddedProfiles.mat'));
if ~isempty(grid_file)
    load(fullfile(grid_file(1).folder,grid_file(1).name));
    
%     %ALB create a NaN mask for epsilon and chi
%     epsi_mask=isnan(grid.epsilon);
%     chi1_mask=isnan(grid.chi1);
%     chi2_mask=isnan(grid.chi2);
%     
%     nonanepsi=fillmissing(grid.epsilon,'linear',2);
%     nonanepsi=fillmissing(nonanepsi,'linear');
%     nonanepsi(nonanepsi<0)=1e-11; % hard coded noisefloor

%ALB keep old version    
%     myFCTD_GridData.epsilon1 = log10(interp2(grid.dnum,grid.z,grid.epsilon,...
%         myFCTD_GridData.time,myFCTD_GridData.depth));
%     myFCTD_GridData.epsilon2 = log10(interp2(grid.dnum,grid.z,grid.epsilon,...
%         myFCTD_GridData.time,myFCTD_GridData.depth));
% ALB no nan
% epsi```````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````
%     myFCTD_GridData.epsilon1 = log10(interp2(grid.dnum,grid.z,nonanepsi,...
%         myFCTD_GridData.time,myFCTD_GridData.depth));
%     myFCTD_GridData.epsilon2 = log10(interp2(grid.dnum,grid.z,nonanepsi,...
%         myFCTD_GridData.time,myFCTD_GridData.depth));
%     myFCTD_GridData.chi1 = log10(interp2(grid.dnum,grid.z,grid.chi1,...
%         myFCTD_GridData.time,myFCTD_GridData.depth));
%     myFCTD_GridData.chi2 = log10(interp2(grid.dnum,grid.z,grid.chi2,...
%         myFCTD_GridData.time,myFCTD_GridData.depth));
    
    
    myFCTD_GridData.tGrid.epsilon1 = log10(interp2(grid.dnum,grid.z,grid.epsilon,...
        myFCTD_GridData.tGrid.time,myFCTD_GridData.tGrid.depth));
    myFCTD_GridData.tGrid.epsilon2 = log10(interp2(grid.dnum,grid.z,grid.epsilon,...
        myFCTD_GridData.tGrid.time,myFCTD_GridData.tGrid.depth));
    myFCTD_GridData.tGrid.chi1 = log10(interp2(grid.dnum,grid.z,grid.chi1,...
        myFCTD_GridData.tGrid.time,myFCTD_GridData.tGrid.depth));
    myFCTD_GridData.tGrid.chi2 = log10(interp2(grid.dnum,grid.z,grid.chi2,...
        myFCTD_GridData.tGrid.time,myFCTD_GridData.tGrid.depth));
end
...
%         hold(handles.(sprintf('Frame%1d',i)),'off');
                                                                                                                        delete(get(handles.(sprintf('Frame%1d',i)),'children'));
        data = grid.(Vars2PlotInFrames{i});
        contourf(handles.(sprintf('Frame%1d',i)),grid.dnum,grid.z,log10(data),...
        linspace(FastCTD_GUI_data.settings.(sprintf('%sMin',VarsBoxNames{i})){3}, ...
                 FastCTD_GUI_data.settings.(sprintf('%sMax',VarsBoxNames{i})){3}, ...
                 FastCTD_GUI_data.settings.ColorCount{3}*2),'linecolor','none');
        hold(handles.(sprintf('Frame%1d',i)),'on');
        contour(handles.(sprintf('Frame%1d',i)), ...
                grid.dnum, ...
                grid.z,...
                grid.sgth, ...
                isopycnal2plot(~isnan(isopycnal2plot)), ...
                'color',[1 1 1]*0.3,'linewidth',1);
%         hold(handles.(sprintf('Frame%1d',i)),'off');
        shading('flat');
    end
