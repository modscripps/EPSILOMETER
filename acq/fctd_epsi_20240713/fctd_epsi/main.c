...
//    int portnum_mod = 0;
	...
	while(fget_str_f(confstr,sizeof(confstr),fp))
	{
       ...
       /* // ADDED mnbui 12March2021 - Altimeter, efe, volt are not in CTD data -> add Altimeter's size string
        if (!strcmp("CTD.Altilength",strPtr))    // Alti's length
        {
            fctd_epsi_ptr->alti_flag = true;  //ALB Add Altiflag to write in the file, even without this flag the acq app will get the EFE blocks.
            strPtr = strtok(NULL,sep);    // get port number
            fctd_epsi_ptr->fish.Altilength = atoi(strPtr);
            printf("Alti = %d\n",fctd_epsi_ptr->fish.Altilength);
        }
        if (!strcmp("CTD.EFElength",strPtr))    // EFE's length
        {
            fctd_epsi_ptr->efe_flag = true;   //ALB Add Efeflag to write in the file, even without this flag the acq app will get the EFE blocks.
            strPtr = strtok(NULL,sep);    // get port number
            fctd_epsi_ptr->fish.Efelength = atoi(strPtr);
        }
        if (!strcmp("CTD.VOLTlength",strPtr))    // VOLT's length
        {
            fctd_epsi_ptr->volt_flag = true;  //ALB Add Voltflag to write in the file
            strPtr = strtok(NULL,sep);    // get port number
            fctd_epsi_ptr->fish.Voltlength = atoi(strPtr);
        }
        if (!strcmp("CTD.VNAVlength",strPtr))    // VOLT's length
        {
            fctd_epsi_ptr->vnav_flag = true;  //ALB Add Voltflag to write in the file
            strPtr = strtok(NULL,sep);    // get port number
            fctd_epsi_ptr->fish.Vnavlength = atoi(strPtr);
        }
        */
...
//		{
//			fctd_epsi_ptr->network_flag = true;
//		}
...
//			if (atoi(strPtr) == 1){
//				fctd_epsi_ptr->dropnumBaseonWinch = TRUE;
//				fctd_epsi_ptr->network.getStatusFish = TRUE;
//			}
//			else fctd_epsi_ptr->ctd.getStatusFish = TRUE;
//		}
...
//			FastCTDPtr->RecHeader.total_drops = atoi(strPtr);
//		}
