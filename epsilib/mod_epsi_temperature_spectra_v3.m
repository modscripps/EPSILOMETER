...
%   calibration
%  - TODO - Find a better way to get the best data from the whole deployment  
%
%
% ALB Feb 2019.
%
...
% length
% id profile
tscanAlternate = floor(0.8*length(Profile.epsitime)/320);
tscanDefault = 50;
...
epsi_T      = length(Profile.epsitime);
ctd_T       = length(Profile.ctdtime);

% make sure all the fields in the CTD structure are columns.
Profile=structfun(@(x) x(:),Profile,'un',0);
%Length_profile=86400*(Profile.epsitime(end)-Profile.epsitime(1));
Length_profile=Profile.epsitime(end)-Profile.epsitime(1); %NC - epsitime is now in seconds
...
% compute fall rate
Profile.w = smoothdata([diff(Profile.P(:))./diff(Profile.ctdtime(:)*86400) ;nan],...
                           'movmean',10);
...
data_CTD = cell2mat(cellfun(@(x) Profile.T(x),ctd_indscan,'un',0)).';
...
if ~isempty(indt1)
    Profile.t1_volt=fillmissing(Profile.t1_volt,'linear');
    Profile.t1_volt=filloutliers(Profile.t1_volt,'center','movmedian',1000);
    
    data_EPSI(indt1,:,:) = cell2mat(cellfun(@(x) filloutliers( ...
                Profile.t1_volt(x),'center','movmedian',5), ...
                 epsi_indscan,'un',0)).';
    P11_epsi(indt1,:,:)=alb_power_spectrum(squeeze(data_EPSI(indt1,:,:)),1./tscan);
end
if ~isempty(indt2)
    Profile.t2_volt=fillmissing(Profile.t2_volt,'linear');
    Profile.t2_volt=filloutliers(Profile.t2_volt,'center','movmedian',1000);

    data_EPSI(indt2,:,:) = cell2mat(cellfun(@(x) filloutliers( ...
        Profile.t2_volt(x),'center','movmedian',5),...
              epsi_indscan,'un',0)).';
...
w=cellfun(@(x) abs(median(Profile.w(x))),ctd_indscan);
...
titleStr=sprintf('%s cast %i - temperature',titleStr,Profile.profNum);
else
  titleStr=sprintf('%s epsitime %i-%i - temperature',titleStr,Profile.epsitime(1),Profile.epsitime(end));  
end
...
if isfield(Profile,'profNum')
filename=sprintf('%sTctd_Tepsi_comp_cast%i_t1.png',Meta_Data.L1path,Profile.profNum);
else
    filename=sprintf('%sTctd_Tepsi_comp_t1.png',Meta_Data.L1path);
end
