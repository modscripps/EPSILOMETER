...
% -------------------------------------------------------------------------
%% USER CHOICES (1/3) - Choose processing steps
% 1 = do this step
...
Meta_Data.PROCESS.Prmin = @(x) nanmin(x) + 0.2*range(x);
Meta_Data.PROCESS.Prmax = @(x) nanmin(x) + 0.8*range(x);
...
if processingSteps(1)
    ...
    load(fullfile(Meta_Data.path_mission,'L1','Meta_Data'));
end
