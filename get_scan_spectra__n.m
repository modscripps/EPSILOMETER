function [scan] = get_scan_spectra__n(Profile)
% Attempting to make compute all these spectra from matrices, instead of
% looping through every scan array
% NC: 4/15/22
%
...
% Find the midpoint indices of each depth bin. You need one array of
% indices for lower-resolution ctd data and another for higher-resolution
% epsi data. 
dz = Meta_Data.PROCESS.dz;
z_array = ceil(min(Profile.ctd.z)):dz:floor(max(Profile.ctd.z));
for iZ=1:numel(z_array)
    [~,ctd_mids_idx(iZ)] = min(abs(Profile.ctd.z-z_array(iZ)));
end
...
%     scan.ktemp  = kt(scan.s,scan.t,scan.pr);
    ...
    for iChan=1:numel(chanList)
        ...
        % Put new variables in the structure
        varList = {'Ps_volt_f','Ps_velocity_f','Ps_shear_k','Ps_shear_co_k','Ps_shear_mv_k',...
            'epsilon','epsilon_co','epsilon_mv','fc','kc','Ppan','Ppan_co','fom','calib_volt','calib_vel'};
        for iVar=1:numel(varList)
            scan.(varList{iVar}).(currChannel(1:2)) = eval(varList{iVar});
        end
